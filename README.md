...
(read from the package by default, so publish them only to add your own):
...
hooks, against the same endpoints this package serves. Nothing requires it — the
endpoints are plain JSON — but it saves reimplementing the hash loop.
...
scarce — a verified phone number, a national ID — where fake accounts are the
concern.
...
as an ISO-8601 stamp. Prefer `expires_in` for a countdown — a device clock that
disagrees with the server cannot throw it off.
...
$rules = ['pow_nonce' => ['required', new ProofOfWork]];
...
optionally the issuing address — a couple of hundred bytes, and no table of its
own. `cache.store` defaults to the application's default store. Two requirements
...
accumulate one entry per abandoned challenge — files against an inode quota, rows
in the cache table — so schedule the sweep:
...
"below roughly 3%" — zero of 100 puts the 95% upper bound near there.
